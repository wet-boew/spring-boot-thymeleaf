...
/**
 * The Class BreadcrumbServiceImpl is responsible for building and retrieving
...
public class BreadcrumbServiceImpl implements BreadcrumbService, Serializable {
	 ...
	 * from the breadcrumb map.
	 */
	private static final int HOME_AND_DEPARTMENT = 2;

	/**
	 * The show landing page breadcrumb property of true states that the
	...
	public List<Object> getBreadCrumbList() {
		...
		// Return the full list as is - css/javascript will have to do the
		// manipulations based on the
		// new breadcrumbs requirements
		// return adjustForBreadcrumbLength(breadCrumbList);
	}

	/**
	 * Validate bread crumb properties. root.key and associated key for root.key
	 * value must exist.
	 *
	 * @return the string
	 */
	...
	/**
	 * Insert home page. Add an Application Home breadcrumb if one doesn't
	 * exist.
	 *
	 * @param rootKey
	 *            the root key
	...
	private void insertHomePage(String rootKey) {
		...
		if (getBreadcrumbMap().size() == HOME_AND_DEPARTMENT) {
			getBreadcrumbMap().put(extractView(rootKey), createBreadCrumb(extractView(rootKey),
	...
	/**
	 * Adjust for bread crumb length. Do not show all breadcrumbs. Show and
	 * elliptical to indicate this.
	 *
	 * @param fullBreadCrumbList
	 *            the full bread crumb list
	...
	/**
	 * Builds the ecliptical crumb. The bread crumb ... indicates that some
	 * breadcrumbs are not shown.
	 *
	 * @param breadCrumbList
	 *            the bread crumb list
	 * @param displayedBreadCrumbList
	 *            the displayed bread crumb list
	 */
	...
	/**
	 * Breadcrumbs configured. Check to see if bread crumbs have been configured
	 * in application.
	 *
	 * @return true, if successful
	 */
	private boolean breadcrumbsConfigured() {
		if (CollectionUtils.isEmpty(breadcrumbMessageSource.getKeys(breadcrumbMessageSource.getBasename(),
				LocaleContextHolder.getLocale()))) {
			return false;
		}
		return true;
	...
	/**
	 * Creates a new bread crumb list containing the first link to Home
	 * "www.canada.ca" in the current locale.
	 *
	 * @return the list
	 */
	...
	private BreadCrumb setBreadCrumbLanguageTitle(String breadcrumbKey, BreadCrumb breadCrumb) {
		...
		if (LocaleContextHolder.getLocale().getLanguage().equals(Locale.CANADA_FRENCH.getLanguage())) {
			...
			if (breadcrumbKey.equals(WETModelKey.CANADA_HOME.name())) {
				...
				localeBreadCrumb.setHref(
						applicationMessageSource.getMessage("department.home.url", null, Locale.CANADA_FRENCH));
			}
		} else {
	 ...
	 *
	 * @param currentView
	 *            the new previously visited bread crumb
	 */
